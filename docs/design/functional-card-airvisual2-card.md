...
In the future, SAK will support card templates, and usage will be something like:
...
??? example "(testing) Swiss Army Knife AirVisual2 layout template"
    ```yaml linenums="1" title="layout-airvisual2-template.yaml"
    sak_layout_airvisual2:
      template:
        type: layout
        defaults: 
          - var_entity_index: 0
